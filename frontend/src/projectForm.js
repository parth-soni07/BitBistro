...
const ProjectForm = () => {
  ...
  async function deployContract(event) {
    ...
    try {
      ...
      const biddingContractFactory = new ethers.ContractFactory(biddingAbi, biddingByteCode, signer);
      const biddingContract = await biddingContractFactory.deploy(projectName, projectDescription, projectMetrics);
      console.log("Bidding Contract deployed");
      setbiddingContractAddress(await biddingContract.address);
      console.log("Current Deployed Bidding Contract Address : ", biddingContractAddress);
      ...
      console.log("Bidding Contract Successfully added to Master with address : ", biddingContractAddress);
    } catch (error) {
